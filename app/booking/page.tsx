...
const serviceIcons: Record<string, string> = {
  ...
  "Brakes": "🛑", "AdBlue & Fluids": "💧", "Inspection": "🔩",
  "MOT Preparation": "🚗", "Air Conditioning": "❄️", 
};
...
interface Service { id: number; name: string; price: string; description: string; }
interface Brand { id: number; name: string; }
interface CarModel { id: number; name: string; brand_id: number; }
interface FormData {
  serviceId: number | null; serviceName: string; showAllServices: boolean;
  brandId: number | null; brandName: string; modelId: number | null; modelName: string;
  year: string; date: string; time: string; firstName: string; lastName: string;
  email: string; phone: string; address: string; deliveryOption: "pickup" | "delivery" | "";
}

const emptyForm: FormData = {
  serviceId: null, serviceName: "", showAllServices: false,
  brandId: null, brandName: "", modelId: null, modelName: "",
  year: "", date: "", time: "", firstName: "", lastName: "",
  email: "", phone: "", address: "", deliveryOption: "",
};
...
export default function BookingPage() {
  ...
  const validate = () => {
    ...
    if (currentStep === 1 && !formData.serviceId) e.service = "Please select a service.";
    if (currentStep === 2) {
  ...
  const handleNext = async () => {
    if (!validate()) return;
    if (currentStep < 4) { setCurrentStep(s => s + 1); return; }
    setLoading(true);
    try {
      const res = await fetch(API + "/api/appointments", {
        ...
        body: JSON.stringify({
          ...
          car_model_id: formData.modelId, car_year: Number(formData.year),
          service_id: formData.serviceId, appointment_date: formData.date,
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center px-4 py-16">
        <div className="bg-white rounded-2xl shadow-lg max-w-md w-full p-10 flex flex-col items-center gap-5 text-center">
          ...
          <p className="text-xs text-gray-400">A confirmation sent to <strong>{formData.email}</strong>.</p>
          <button onClick={() => { setSubmitted(false); setCurrentStep(1); setFormData(emptyForm); }}
          ...
          {currentStep === 2 && (
            <div className="mb-6">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div>
                  <select value={formData.modelId ?? ""} disabled={!formData.brandId}
                    className={inputClass(errors.model) + " disabled:opacity-50 disabled:cursor-not-allowed"}
                    onChange={e => { const m = models.find(x => x.id === Number(e.target.value)); update("modelId", Number(e.target.value)); update("modelName", m?.name || ""); }}>
              ...
              <div className="flex flex-col gap-4">
                <div>
                  <input type="date" value={formData.date} min={new Date().toISOString().split("T")[0]}
                    onChange={e => update("date", e.target.value)} className={inputClass(errors.date)} />
                  {errors.date && <p className="text-red-500 text-xs font-semibold mt-1">{errors.date}</p>}
                </div>
                <div>
                  <select value={formData.time} onChange={e => update("time", e.target.value)} className={inputClass(errors.time)}>
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-col gap-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <input type="text" placeholder="First Name*" value={formData.firstName} onChange={e => update("firstName", e.target.value)} className={inputClass(errors.firstName)} />
                    {errors.firstName && <p className="text-red-500 text-xs font-semibold mt-1">{errors.firstName}</p>}
                  </div>
                  <div>
                    <input type="text" placeholder="Last Name*" value={formData.lastName} onChange={e => update("lastName", e.target.value)} className={inputClass(errors.lastName)} />
                    {errors.lastName && <p className="text-red-500 text-xs font-semibold mt-1">{errors.lastName}</p>}
                  </div>
                </div>
                <div>
                  <input type="email" placeholder="Email*" value={formData.email} onChange={e => update("email", e.target.value)} className={inputClass(errors.email)} />
                  {errors.email && <p className="text-red-500 text-xs font-semibold mt-1">{errors.email}</p>}
                </div>
                <div>
                  <input type="tel" placeholder="Phone*" value={formData.phone} onChange={e => update("phone", e.target.value)} className={inputClass(errors.phone)} />
                  {errors.phone && <p className="text-red-500 text-xs font-semibold mt-1">{errors.phone}</p>}
                </div>
                <div>
                  <input type="text" placeholder="Address*" value={formData.address} onChange={e => update("address", e.target.value)} className={inputClass(errors.address)} />
              ...
              <div className="mt-6 bg-gray-50 rounded-xl border border-gray-100 p-5">
                ...
                {[
                  ...
                  <div key={row.label} className="flex gap-2 text-sm text-gray-600 pb-2 mb-2 border-b border-gray-100 last:border-b-0 last:mb-0 last:pb-0">
                    <span className="font-bold text-[#1a2b4a] min-w-[90px]">{row.label}:</span>
                    <span>{row.value}</span>
                  </div>
                ))}
              </div>
