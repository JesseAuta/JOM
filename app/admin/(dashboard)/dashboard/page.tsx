...
export default function Dashboard() {
  ...
  const mapBooking = (item: any): Booking => ({
    ...
    carBrand: item.carBrand || item.car_brand || '',
    carModel:
      item.carModel ||
      item.car_model ||
      (item.car_model_id ? String(item.car_model_id) : ''),
    name: item.name || item.first_name || '',
    lastName: item.lastName || item.last_name || '',
    email: item.email || '',
    year: item.year || (item.car_year ? String(item.car_year) : ''),
    address: item.address || '',
    telephone: item.telephone || item.phone || '',
    pickup: Boolean(item.pickup ?? item.pickup_required),
    delivery: Boolean(item.delivery ?? item.delivery_required),
    service:
      item.service ||
      item.service_name ||
      (item.service_id ? String(item.service_id) : ''),
    date: item.date || item.booking_date || '',
    time: item.time || item.booking_time?.slice(0, 5) || '',
    mechanic:
      item.mechanic ||
      item.mechanic_name ||
      (item.mechanic_id ? String(item.mechanic_id) : ''),
    status: (item.status as Status) || 'new',
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      const updatedBooking = mapBooking(res.data);
  ...
  const masters = [...new Set(bookings.map((b) => b.mechanic).filter(Boolean))];
  ...
  return (
    <div className='space-y-6 px-4 sm:px-6 lg:px-8'>
      ...
      <div className='space-y-4 rounded-xl bg-white p-6 shadow'>
        <input
          placeholder='Search by number plate...'
          className='w-full rounded-lg border border-amber-300 p-3'
          value={search}
          onChange={(e) => setSearch(e.target.value)}
        />
      ...
      <div className='hidden overflow-x-auto rounded-xl bg-white shadow lg:block'>
        <table className='min-w-250 w-full table-auto text-sm'>
          <thead className='bg-gray-100 text-left'>
            <tr>
      ...
      {editBooking && (
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/40'>
          <div className='mx-4 max-h-[90vh] w-full max-w-md space-y-4 overflow-y-auto rounded-xl bg-white p-6'>
            <h3 className='text-lg font-semibold'>
              Edit Booking: {editBooking.numberPlate} ({editBooking.name}{' '}
              {editBooking.lastName})
            </h3>
...
function StatusBadge({ status }: { status: Status }) {
  ...
  return (
    <span className={`rounded px-2 py-1 text-xs text-white ${styles[status]}`}>
      {labels[status]}
    </span>
  );
}
